...
import com.dicoding.picodiploma.treasurehunt_kotlin.databinding.ActivityHuntMainBinding
...
class HuntMainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val have_continue = intent.getBooleanExtra("continue",false)
        if(have_continue == true)
            viewModel.updateHint(viewModel.nextGeofenceIndex())

        binding.found.setOnClickListener {

            val intent:Intent;
            if(have_continue == true)
                intent = Intent(this, ClueSuccessActivity::class.java)
            else
                intent = Intent(this, ClueResultActivity::class.java)
            intent.putExtra("elapsedTime", elapsedTime)
            viewModel.geofencedecResourceId.observe(this, { value ->
                Log.d("LiveDataValue", "Value changed: $value")
               intent.putExtra("description",getString(value))
            })
    ...
    fun runCountClock(){
        Thread {
            ...
            while (true) {
                ...
                runOnUiThread {
                    // Print the count-up clock
                    val hours = elapsedTime / 3600
                    val minutes = (elapsedTime % 3600) / 60
                    val seconds = elapsedTime % 60

                    binding.counttimer.text = String.format("%02d:%02d:%02d", hours, minutes, seconds)
                }
            }
        }.start()
